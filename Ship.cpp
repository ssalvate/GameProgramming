...
void Ship::UpdateActor(float deltaTime)
{
	...
	// Update positions based on speed and deltaTime
...
void Ship::ProcessKeyboard(const uint8_t* state)
{
	...
	// right/left
	if (state[SDL_SCANCODE_D]) mRightSpeed += 250.0f;
	if (state[SDL_SCANCODE_A]) mRightSpeed -= 250.0f;
	
	// up/down
	if (state[SDL_SCANCODE_S]) mDownSpeed += 300.0f;
	if (state[SDL_SCANCODE_W]) mDownSpeed -= 300.0f;
